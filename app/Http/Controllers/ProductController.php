...
class ProductController extends Controller
{
    ...
    public function show($id)
    {
      ...
      ->select(DB::raw('products.id, products.name AS prodname, price, description, discount, stock, categories.name AS catname, COUNT(reviews.id) AS numRatings, AVG(reviews.rating) AS rating'))->where('products.id', $id)->groupBy('products.id', 'products.name', 'products.price', 'products.description', 'products.discount', 'products.stock', 'categories.name')->first();
