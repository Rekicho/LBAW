...
CREATE TABLE client (
    id INTEGER REFERENCES user (id) PRIMARY KEY ON UPDATE CASCADE,
...
CREATE TABLE report_treatment (
    report_id INTEGER REFERENCES report (id) PRIMARY KEY ON UPDATE CASCADE,
    id_staff_member INTEGER NOT NULL REFERENCES staff_member (id) ON UPDATE CASCADE,
...
CREATE TABLE purchase (
    id SERIAL PRIMARY KEY,
    "date" TIMESTAMP WITH TIME zone DEFAULT now() NOT NULL,
    id_billing_information INTEGER NOT NULL REFERENCES billing_information (id) ON UPDATE CASCADE,
    id_client INTEGER NOT NULL REFERENCES client (id) ON UPDATE CASCADE
);
...
CREATE TABLE purchased_product (
    id_product INTEGER NOT NULL REFERENCES product (id) ON UPDATE CASCADE,
    id_purchase INTEGER NOT NULL REFERENCES purchase (id) ON UPDATE CASCADE,
    name TEXT NOT NULL,
...
CREATE TABLE purchase_transition (
    id SERIAL PRIMARY KEY,
    "date" TIMESTAMP WITH TIME zone DEFAULT now() NOT NULL,
    id_purchase_state INTEGER NOT NULL REFERENCES purchase_state (id) ON UPDATE CASCADE,
    id_purchase INTEGER NOT NULL REFERENCES purchase (id) ON UPDATE CASCADE
);

CREATE TABLE toggle_active (
    id_staff_member INTEGER NOT NULL REFERENCES staff_member (id) ON UPDATE CASCADE,
    id_client INTEGER NOT NULL REFERENCES client (id) ON UPDATE CASCADE,
    "date" TIMESTAMP WITH TIME zone DEFAULT now() NOT NULL,
